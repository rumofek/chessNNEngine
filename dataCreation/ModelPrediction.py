...
model = load_model("/media/ConvAFNONEF32K3S1_Flatten_D2400AFTANH_FinalSigmoid.h5")
...
# top5 = {}
# for movePos in range(len(prediction)):
#     top5[movePos] = prediction[movePos]
#     if len(top5) > 7:
#         keys = list(top5.keys())
#         minKey = keys[0]
#         for key in keys:
#             if top5[key] < top5[minKey]:
#                 minKey = key
#         top5.pop(minKey)
...
# for keys in top5.keys():
#     print(uciMoves[keys], prediction[keys])

sorted = sortIndexes(prediction)

for i in range(10):
    print(uciMoves[sorted[i]], prediction[sorted[i]])
...
print(getLegalMove(sorted, chessboard))
